...
export default function Home() {
  return (
    <div className="space-y-8">
      ...
      {miniGames.length > 0 ? (
        <div className={`grid grid-cols-1 ${miniGames.length > 1 ? 'md:grid-cols-2 lg:grid-cols-3' : 'md:grid-cols-1 lg:grid-cols-1'} gap-6 auto-rows-fr`}>
          {miniGames.map((game) => (
            <Card key={game.id} className={`flex flex-col overflow-hidden shadow-xl hover:shadow-2xl transition-shadow duration-300 ease-in-out bg-card rounded-xl ${miniGames.length === 1 ? 'max-w-lg mx-auto' : ''}`}>
        ...
        <p className="text-center text-muted-foreground">No games available at the moment. Check back soon!</p>
      )}
